...
const EMPTY_WORKFLOW_UPDATE = {
  workflow_id: '',
  document_view_accessor: '',
}

export default function WorkflowUpdates({ updates, onChange }) {
  const update = (idx, updated) => {
    onChange(updates.map((u, i) => (i === idx ? updated : u)))
  }

  const add = () => onChange([...updates, { ...EMPTY_WORKFLOW_UPDATE }])
      ...
      {updates.map((u, idx) => (
        <div key={idx} className="group flex gap-3 items-end p-3 rounded-lg border border-gray-200 bg-white hover:border-gray-300 transition-colors">
          <TextField
            label="Workflow ID"
            value={u.workflow_id}
            onChange={(v) => update(idx, { ...u, workflow_id: v })}
